...
if (process.env.CI_COMMIT_REF_SLUG) {
  dynamicConfig.baseUrl = `https://${process.env.CI_COMMIT_REF_SLUG}.gavinvaught.dev`;

  if (process.env.CI_COMMIT_REF_SLUG === 'master') {
    dynamicConfig.baseUrl = 'https://gavinvaught.dev';
  }
}
...
exports.config = Object.assign({}, {
  ...
  specs: [
    ...
    (process.env.MAINTENANCE_MODE) ?
      './test/specs/**/maintenanceModeCheck.spec.js' :
      './test/specs/**/*.spec.js',
  ],
  // Patterns to exclude.
  exclude: [
  ...
  */
  onPrepare: function (config, capabilities) {
    const seleniumAssistant = require('selenium-assistant');
    const browsers = seleniumAssistant.getLocalBrowsers();
    config.capabilities = browsers.map(browser => ({
      browserName: browser.getId()
    }));
  },
  /**
